...
脚本支持交互选择 Docker 本地部署或 Cloudflare Workers 独立部署：
...
Cloudflare Workers 部署统一使用 API Token。请先准备具备 Workers 和 D1 创建/编辑权限的 Cloudflare API Token，运行脚本时会提示输入。
...
选择 `Cloudflare Workers 部署` 后，按提示输入 Cloudflare API Token；如果本机没有 Node.js，请在运行环境提示中输入 `docker`。
