...
- **鉴权**：支持直接 `Authorization: Bearer <MCP_TOKEN>`，也支持 OAuth 2.0 Client Credentials 换取短期 access token
- **浏览器 Origin 限制**：如需允许浏览器来源访问，在后台填写允许来源
- **客户端地址**：`https://your-litemark.example.com/mcp/`

### `GET /.well-known/oauth-protected-resource`
- **描述**：OAuth 2.0 Protected Resource Metadata，用于 MCP 客户端发现授权服务器
- **鉴权**：不需要

### `GET /.well-known/oauth-authorization-server`
- **描述**：OAuth 2.0 Authorization Server Metadata
- **鉴权**：不需要

### `POST /oauth/token`
- **描述**：OAuth 2.0 Token Endpoint，支持 `client_credentials`
- **Content-Type**：`application/x-www-form-urlencoded`
- **参数**：
  ```text
  grant_type=client_credentials
  client_id=litemark-mcp
  client_secret=<MCP_TOKEN>
  scope=bookmarks:read bookmarks:write
  resource=https://your-litemark.example.com/mcp/
  ```
- **响应**：
  ```json
  {
